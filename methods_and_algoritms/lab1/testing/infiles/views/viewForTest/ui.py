...
class UIManager:
    ...
    def create_task_buttons(self):
        for task_number in self.task_manager.tasks.keys():
            self.grid.controls.append(
                ft.TextButton(text=task_number, on_click=self.set_task, width=80, height=80)
            )
    ...
    def add_variable(self, e):
        ...
        try:
            value = float(self.txt_input.value) if self.txt_input.value else 1  # Дефолтне значення = 1
