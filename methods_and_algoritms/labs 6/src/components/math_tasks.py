...
def plot_newton_interpolation(points: Points, num_points: int = 200, title: str = "Newton Interpolation and Derivative"):
    ...
    x_min, x_max = min(x_values_input), max(x_values_input)
    x_range = x_max - x_min
    padding = 0.1 * x_range  # 10% на кожну сторону
    x_min -= padding
    x_max += padding
    
    # Генеруємо рівномірно розподілені x-значення для гладкого графіка
    x_values = numpy.linspace(x_min, x_max, num_points)
    y_values = []  # Значення полінома
    derivative_values = []  # Значення похідної
    
    # Обчислюємо значення полінома та похідної для кожної точки
    for x in x_values:
        poly_val, deriv_val = _evaluate_newton_polynomial_and_derivative(coeffs, x_values_input, x)
        y_values.append(poly_val)
        derivative_values.append(deriv_val)
    ...
    # Графік полінома
    plt.subplot(2, 1, 1)
    plt.plot(x_values, y_values, 'b-', label="Newton Polynomial")
    plt.plot(x_values_input, y_values_input, 'ro', label="Data Points")  # Початкові точки
    plt.title(f"{title} - Polynomial")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True)
    ...
    # Графік похідної
    plt.subplot(2, 1, 2)
    plt.plot(x_values, derivative_values, 'g-', label="Derivative")
